...
const HodApprovalDashboard = () => {
  ...
  const fetchRequests = async () => {
    try {
      ...
      const pendingRequests = data.filter(request => !request.hodApproved);
  ...
  const handleApprove = async (leaveRequestId) => {
    try {
      ...
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:8080/leave-requests/hod/${leaveRequestId}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 sm:p-6 lg:p-8">
      <div className="max-w-6xl mx-auto bg-white rounded-lg shadow-sm">
        <div className="p-6 border-b border-gray-200">
          <h2 className="text-2xl font-bold text-gray-900">HOD - Pending Leave Approvals</h2>
          <p className="mt-1 text-gray-500">Review and manage leave requests</p>
        </div>
        <div className="p-6">
